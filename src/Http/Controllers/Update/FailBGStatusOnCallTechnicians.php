...
class FailBGStatusOnCallTechnicians extends Controller
{
   public function failbackgroundstatus(Request $request) {
       ...
       $updated = DB::connection("conn_oncalls")->table("oncall_technicians")
        ...
        if($updated) {
            return [
                "success"   => true,
                "message"   => "Successfully set to failed background status the on call technicians."
            ];
        }
        else {
            return [
                "success"   => false,
                "message"   => "Failed to set failed background status to the on call technicians."
            ];
        }
    }
}